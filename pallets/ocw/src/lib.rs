...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			let price_str = str::from_utf8(&price);
			let parsed_price = match Self::parse_price(&price_str) {
				Some(parsed_price) => Ok(parsed_price),
				None => {
					log::warn!("Unable to extract price : {:?}", price_str);
					Err(<Error<T>>::ParsePriceError)
				}
			}?;

			// 不具签名但对payload进行签名
			/* 获取波卡价格不涉及用户账户与交易安全，因此不需要签名交易；
			   但是获取波卡价格有类似预言机的功能，通过 payload 签名可获得较为有序、稳定的交易过程。
			*/
		...
		fn fetch_price() -> Result<Vec<u8>, Error<T>> {
			...
			// 利用 serde_json 对抓取的数据进行value解析。
			let v: serde_json::Value = serde_json::from_str(&body_str).map_err(|_| <Error<T>>::HttpFetchingError)?;

			// 匹配出priceUsd字段值，并转为Vec<u8>类型
			let price_usd: Vec<u8> = v["data"]["priceUsd"].as_str().unwrap().as_bytes().to_vec();

			Ok(price_usd)
		}

		/// 传入从 COINCAP 上获取的String类型USD价格，将其解析为整数u64，小数Permill格式
